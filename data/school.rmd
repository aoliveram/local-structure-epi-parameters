...
students     <- fread("data-raw/pone.0153690.s001.csv") #678
interactions <- fread("data-raw/pone.0153690.s003.csv") #10781
```
...
# Starting point
startpoint <- c(
  edges = 0, nodematch.gender = -10.478966226607,
  nodefactor.grade.8 = 2.0692262820586, 
  nodematch.grade = 8.81811181083116,
  balance = -0.00882693212436488, 
  nodematch.lunch = -9.19321031538299
  )

fit3r <- ergm(
  net2 ~ edges  + nodematch('gender') + nodefactor('grade') +
    nodematch('grade') + balance + nodematch('lunch') + altkstar(0.5, fixed=TRUE), 
    constraints = ~ edges #,
...
```{r}
sim1000 <- lapply(
  sim1000, network::delete.vertex.attribute, attrname = "unique"
  )

sim1000 <- lapply(
  sim1000, network::delete.vertex.attribute, attrname = "initialsNum"
  )

saveRDS(
  sim1000,
  file = "~/school/Simulated_1000_networks.rds",
  compress = FALSE
  )
```
