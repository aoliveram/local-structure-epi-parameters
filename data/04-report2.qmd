...
summRt <- function(val) {
  ...
  # Average of the ERGM network
  avgs <- simstats[,
    list(avg = mean(.SD[[1]], na.rm = TRUE)), by = "nettype",
    .SDcols = val]

  # Average of the ERGM network
  avg_ergm <- mean(
    simstats[nettype == "ergm", .SD[[1]], .SDcols = val],
    na.rm = TRUE
    )

  ans <- ggplot(simstats, aes_string(x = val)) +
    geom_histogram() +
    ...
    geom_vline(
      aes(xintercept = avg), avgs,
      colour = "blue", linetype = 2
      ) +
    # Label to the vline "ERGM mean"
    geom_text(
      aes(x = 0.5, y = 0.5, label = "ERGM mean"),
...
#| echo: false
#| label: peak-time
simstats[, table(nettype, peak_time)] |> knitr::kable()

ggplot(simstats, aes(x = peak_time)) +
