...
```{r}
ggplot(simstats, aes(x = ergm_twopath)) +
  geom_histogram() +
  facet_wrap(~nettype) +
  labs(
    x = "Two-path",
...
ggplot(simstats, aes(x = ergm_balance)) +
  geom_histogram() +
  facet_wrap(~nettype) +
  labs(
    x = "Balance",
...
ggplot(simstats, aes(x = ergm_twopath, y = ergm_balance)) +
...
#| label: models-fe
#| output: asis
baseline <- ~ I(factor(nettype)) + is_smallworld 
...
#| output: asis
#| label: models-simple
terms2add <- "+ ergm_triangle + log(igraph_density) + ergm_twopath + igraph_transitivity"
...
# Removing some params
vars <- setdiff(vars, c("ergm_isolates", "igraph_avg_closeness", "igraph_avg_path_length", "ergm_nodematch.gender", "ergm_nodematch.grade", "ergm_edges", "igraph_density", "igraph_avg_degree"))

baseline <- paste(
  "~ I(factor(nettype)) + is_smallworld + log(igraph_density) + ",
  paste(vars, collapse = " + ")
  ) |>
  as.formula()
